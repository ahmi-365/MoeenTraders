      ...
      <View style={styles.modalOverlay}>
        <View style={styles.modalContainer}>
          <ScrollView showsVerticalScrollIndicator={false}>
            <Text style={styles.modalTitle}>Return Details (Ref P.Inv: {entry.invoiceNo})</Text>

            <DetailRow label="Return Date:" value={entry.returnDate} />
            <DetailRow label="Supplier ID:" value={entry.supplierId} />
            <DetailRow label="Original Purchase ID:" value={entry.originalPurchaseId} />
            ...
            <DetailRow label="Orig. Pymt Method:" value={entry.originalPurchasePaymentMethod} />
...
export default function PurchaseReturnEntryReportScreen() {
  ...
  const [selectedEntry, setSelectedEntry] = useState<PurchaseReturnEntry | null>(null);
  ...
  const fetchEntries = useCallback(async (page: number, isInitialLoad: boolean = false) => {
    ...
    try {
      const response = await axios.get(apiUrl, {
        headers: { Accept: 'application/json' /* Add Auth Token if needed */ },
      });

      if (response.data && response.data.data) {
        const fetchedData: PurchaseReturnEntry[] = response.data.data.map((item: any) => {
          ...
          return {
            ...
            vehicleNumber: originalPurchaseData.vehicle_number?.toString() ?? 'N/A',
            driverName: originalPurchaseData.driver_name?.toString() ?? 'N/A',
            driverContact: originalPurchaseData.driver_contact?.toString() ?? 'N/A',
            originalPurchasePaymentMethod: originalPurchaseData.payment_method?.toString() ?? 'N/A',
          };
        });

        setEntries(prev => (isInitialLoad ? fetchedData : [...prev, ...fetchedData]));
  ...
  const handleLoadMore = useCallback(() => {
    if (currentPage < totalPages && !isFetchingMore && !isLoading) {
      console.log('Loading more entries:', { currentPage: currentPage + 1, totalPages });
        ...
        <View style={styles.tableWrapper}>
          <ReusableTable<PurchaseReturnEntry>
            ...
            emptyStateConfig={{
              ...
              emptyMessage: 'There are no purchase return entries to display for the selected criteria.',
              emptyIcon: '📋'
            }}
            ...
            <View style={styles.loadMoreButtonContainer}>
              ...
              >
                <Text style={styles.loadMoreButtonText}>
                  Load More ({entries.length} of {totalPages > 1 ? `${totalPages} pages` : 'all entries'})
                </Text>
              </TouchableOpacity>
            </View>
          )}
