        ...
        <View style={styles.modalContainer}>
          <ScrollView showsVerticalScrollIndicator={false}>
            <Text style={styles.modalTitle}>
              Details: {entry.expenseReason}
            </Text>

            <Text style={styles.modalSectionTitle}>Expense Details</Text>
            <DetailRow label="Expense ID:" value={entry.actionableId} />
            <DetailRow label="Amount:" value={entry.amount} />
            <DetailRow label="Date of Expense:" value={entry.dateOfExpense} />
            <DetailRow label="Expense Type ID:" value={entry.expenseTypeId} />
            <DetailRow label="Note:" value={entry.note} />

            <View style={styles.modalSeparator} />

            <Text style={styles.modalSectionTitle}>Bank Details</Text>
            <DetailRow label="Name:" value={entry.bankName} />
            <DetailRow label="Account No:" value={entry.bankAccountNumber} />
            <DetailRow
              label="Account Holder:"
              value={entry.bankAccountHolder}
            />
            <DetailRow label="IBAN:" value={entry.bankIban} />
            <DetailRow label="Raast ID:" value={entry.bankRaastId} />
            <DetailRow
              label="Opening Balance:"
              value={entry.bankOpeningBalance}
            />
            <DetailRow
              label="Current Balance:"
              value={entry.bankCurrentBalance}
            />
...
export default function ExpensesEntryReportScreen() {
  ...
  const transformApiData = (apiItem: any): ExpenseEntry => {
    const actionableData = apiItem.actionable || {};
    const expenseTypeData = actionableData.expense_type || {};
    const bankData = actionableData.bank || {};
    const adminData = apiItem.admin || {};

    return {
      expenseReason: expenseTypeData.name?.toString() ?? "N/A",
      amount: formatPKRCurrency(actionableData.amount),
      ...
      dateOfExpense: formatDateOrNA(actionableData.date_of_expense, "date"),
      ...
      expenseTypeId: actionableData.expense_type_id,
      bankId: actionableData.bank_id,
      bankName: bankData.name?.toString() ?? "N/A",
      bankAccountNumber: bankData.account_number?.toString() ?? "N/A",
      bankAccountHolder: bankData.account_holder?.toString() ?? "N/A",
      bankIban: bankData.iban?.toString() ?? "N/A",
      bankRaastId: bankData.raast_id?.toString() ?? "N/A",
      bankOpeningBalance: formatPlainCurrency(bankData.opening_balance),
      bankCurrentBalance: formatPlainCurrency(bankData.current_balance),
      bankCreatedAt: formatDateOrNA(bankData.created_at, "datetime"),
      bankUpdatedAt: formatDateOrNA(bankData.updated_at, "datetime"),
    };
  };
...
const fetchEntries = useCallback(
  async (page: number, isInitialLoad: boolean = false) => {
    ...
    let apiUrl = `https://dewan-chemicals.majesticsofts.com/api/reports/data-entry/expense?page=${page}`;
    if (startDate && endDate) {
      apiUrl += `&start_date=${formatApiDate(startDate)}&end_date=${formatApiDate(endDate)}`;
    }

    // unique cache key for expense
    const cacheKey = `expense-${page}-${startDate ? formatApiDate(startDate) : "null"}-${endDate ? formatApiDate(endDate) : "null"}`;
  ...
  const handleRowPress = (item: ExpenseEntry) => {
  ...
  const columns: Column<ExpenseEntry>[] = [
    {
      ...
      flex: 1.5,
      render: (item) => (
        <Text style={styles.reasonText}>{item.expenseReason}</Text>
      ),
    },
    {
      key: "amount",
    ...
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.appBar}>
          <Text style={styles.appBarTitle}>Expense Entry Report</Text>
        </View>
        ...
        <View style={styles.tableWrapper}>
          <ReusableTable<ExpenseEntry>
            ...
            emptyStateConfig={{
              ...
              emptyMessage: "No expense logs match the selected criteria.",
              emptyIcon: "🧾",
            }}
          />
...
const styles = StyleSheet.create({
  ...
  reasonText: { color: "#0D47A1", fontWeight: "600", fontSize: 13 },
