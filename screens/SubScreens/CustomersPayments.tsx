...
interface CustomerPaymentEntry {
  ...
  customerRecordCreatedAt: string;
  customerRecordUpdatedAt: string;
}
...
const DetailsModal = ({
  ...
  visible: boolean;
  onClose: () => void;
  entry: CustomerPaymentEntry | null;
}) => {
      ...
      <View style={styles.modalOverlay}>
        <View style={styles.modalContainer}>
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            <DetailRow label="Sale Ref ID:" value={entry.saleId} />
            <DetailRow label="Sale Return Ref ID:" value={entry.saleReturnId} />
            <DetailRow
            ...
            <DetailRow label="Mobile:" value={entry.customerMobile} />
            <DetailRow label="Email:" value={entry.customerEmail} />
            <DetailRow label="Address:" value={entry.customerAddress} />
            <DetailRow
              label="Opening Balance:"
              value={entry.customerOpeningBalance}
            />
...
export default function CustomerPaymentEntryReportScreen() {
  ...
  const [selectedEntry, setSelectedEntry] =
    useState<CustomerPaymentEntry | null>(null);
  ...
  const transformApiData = (apiItem: any): CustomerPaymentEntry => {
    const actionableData = apiItem.actionable || {};
    const customerData = actionableData.customer || {};
    ...
    return {
      ...
      saleId: actionableData.sale_id,
      saleReturnId: actionableData.sale_return_id,
      actionableCreatedAt: formatDateOrNA(actionableData.created_at),
      actionableUpdatedAt: formatDateOrNA(actionableData.updated_at),
      customerId: customerData.id ?? actionableData.customer_id,
      customerEmail: customerData.email?.toString() ?? "N/A",
      customerMobile: customerData.mobile?.toString() ?? "N/A",
      customerAddress: customerData.address?.toString() ?? "N/A",
      customerOpeningBalance: formatCurrency(customerData.opening_balance),
      customerRecordCreatedAt: formatDateOrNA(customerData.created_at),
      customerRecordUpdatedAt: formatDateOrNA(customerData.updated_at),
    };
  };
...
const fetchEntries = useCallback(
  async (page: number, isInitialLoad: boolean = false) => {
    ...
    let apiUrl = `https://dewan-chemicals.majesticsofts.com/api/reports/data-entry/customer-payment?page=${page}`;
    if (startDate && endDate) {
      apiUrl += `&start_date=${formatApiDate(startDate)}&end_date=${formatApiDate(endDate)}`;
    }

    // Unique cache key for customer-payment entries
    const cacheKey = `customerPayment-${page}-${startDate ? formatApiDate(startDate) : "null"}-${endDate ? formatApiDate(endDate) : "null"}`;
  ...
  const handleRowPress = (item: CustomerPaymentEntry) => {
  ...
  // --- Column Definition ---
  const columns: Column<CustomerPaymentEntry>[] = [
    {
      key: "trx",
      title: "TRX No.",
      flex: 1.2,
    ...
    { key: "customerName", title: "Customer", flex: 1.5 },
    {
      key: "amount",
      title: "Amount",
      flex: 1,
    ...
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.appBar}>
          <Text style={styles.appBarTitle}>Customer Payment Report</Text>
        </View>
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        <View style={styles.tableWrapper}>
          <ReusableTable<CustomerPaymentEntry>
            data={entries}
            columns={columns}
            ...
            emptyStateConfig={{
              ...
              emptyMessage:
                "No customer payment logs match the selected criteria.",
              emptyIcon: "💳",
            }}
          />
