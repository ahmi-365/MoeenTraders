      ...
      <View style={styles.modalOverlay}>
        <View style={styles.modalContainer}>
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            <Text style={styles.modalSectionTitle}>Supplier Information</Text>
...
export default function SupplierEntryReportScreen() {
  ...
  const [selectedEntry, setSelectedEntry] = useState<SupplierEntry | null>(
    null
  );
  ...
  const transformApiData = (apiItem: any): SupplierEntry => {
    ...
    return {
      ...
      companyName: actionableData.company_name?.toString() ?? "N/A",
...
const SUPPLIER_CACHE_KEY = "supplier_entries";
...
const fetchEntries = useCallback(
  async (page: number, isInitialLoad: boolean = false) => {
    ...
    let apiUrl = `https://dewan-chemicals.majesticsofts.com/api/reports/data-entry/supplier?page=${page}`;
    ...
    try {
      ...
      if (hasInternet) {
        ...
        // ---- OFFLINE MODE ----
        console.log("No internet, loading cached supplier data...");
  ...
  [startDate, endDate, isFetchingMore, entries] // include entries in deps
);
  ...
  const handleRowPress = (item: SupplierEntry) => {
  ...
  const columns: Column<SupplierEntry>[] = [
    {
      ...
      flex: 1.5,
      render: (item) => (
        <Text style={styles.supplierNameText}>{item.name}</Text>
      ),
    },
    { key: "mobile", title: "Mobile", flex: 1.2 },
    ...
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.appBar}>
          <Text style={styles.appBarTitle}>Supplier Entry Report</Text>
        </View>
      ...
      >
        <View style={styles.tableWrapper}>
          <ReusableTable<SupplierEntry>
            ...
            emptyStateConfig={{
              emptyTitle: "No Supplier Entries Found",
              emptyMessage: "No supplier logs match the selected criteria.",
              emptyIcon: "🏭",
            }}
          />
...
const styles = StyleSheet.create({
  ...
  supplierNameText: { color: "#0D47A1", fontWeight: "600", fontSize: 13 },
